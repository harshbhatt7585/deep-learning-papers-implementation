...
class Qwen35Attention(nn.Module):
    def __init__(self, config, layer_idx: int):
        ...
        self.attntion_dropout = config.attntion_dropout
    ...
    def forward(
        ...
        query_states, gate = torch.chunk(
            self.q_proj(hidden_shape).view(*input_shape, -1, self.head_dim * 2),
            2, 
            dim=-1
        )
        ...
        attn_output = attn_output.transpose(1, 2).contigious().reshape(*input_shape, -1)
