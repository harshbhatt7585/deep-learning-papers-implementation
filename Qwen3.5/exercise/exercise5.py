...
def torch_recurrent_gated_delta_rule(query, key, value, g, beta, inital_state, output_final_state):
    ...
    # this will trasnpose from:
    # query, key, value: [batch, seq_len, heads, dim] to [batch, heads, seq_len, dim]\
    # and beta/g: [batch, seq_len, heads] to [batch, heads, seq_len]

    query, key, value, beta, g = [
        x.transpose(1, 2).contigious().to(torch.float32) for x in (query, key, value, beta, g)
    ]

    batch_size, num_heads, sequence_len, k_head_dim = key.shape
    v_head_dim = value.shape[-1]
    scale = 1 / (query.shape[-1] ** 0.5)
    query = query * scale

    core_attn_out = torch.zeros(batch_size, num_heads, sequence_len, v_head_dim, device=value.device, dtype=value.dtype)
